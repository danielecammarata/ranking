...
router.post('/match/delete/:match_id', async (req, res) => {
  ...
  await Match.findById(id, (err, doc) => {
    ...
    query.exec((err, docs) => {
      ...
      docs.forEach(match => {
        ...
        users[match.teamHome.defender.slug].points = newMatch.teamHome.defender
        users[match.teamHome.striker.slug].points = newMatch.teamHome.striker
        users[match.teamAway.defender.slug].points = newMatch.teamAway.defender
        users[match.teamAway.striker.slug].points = newMatch.teamAway.striker

        updateMatches.push(
          Object.assign({},
            newMatch.match,
            {
              difference: newMatch.difference,
              slug: newMatch.slug,
      ...
      updateMatches.forEach(matchData => {
        const newMatch = new Match(matchData)
        newMatch.save(function (err) {})
      })

      Object.keys(users).map(key => {
        User.findOneAndUpdate({_id: users[key].id}, { points: users[key].points })
      })

      docs.forEach(matchData => {
        Match.findById(matchData._id).remove().exec()
      })
...
const createMatchFromOriginal = (original, users) => {
  ...
  const matchData = {
    match: match,
    teamHome: {
      defender: rank.homeDefense,
      striker: rank.homeStriker
    },
    teamAway: {
      defender: rank.awayDefense,
      striker: rank.awayStriker
    },
    badges,
    slug,
    createdAt,
